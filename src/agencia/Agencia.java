...
public class Agencia {
	...
	public void rondaEncuentrosLaborales() {
		...
		double puntaje;
		Usuario_puntaje auxPEmpleado,auxPEmpleador;
		for (int i=0; i < empleadores.size(); i++) {
			auxEmpleador=empleadores.get(i);
			
			for (int j=0; j < empleadosPretensos.size(); j++) {
				auxEmpleado=empleadosPretensos.get(i);
				puntaje = auxEmpleador.calculoPuntajesEmpleador(auxEmpleado.getTicket().getFormulario());
				auxPEmpleado = new Usuario_puntaje(auxEmpleado,puntaje); //guarda empleado con el puntaje
				auxEmpleador.getTicket().nuevoEmpleadoMatcheado(auxPEmpleado);
				
				auxPEmpleador = new Usuario_puntaje(auxEmpleador,puntaje); //guarda empleador con el puntaje
				auxEmpleado.getTicket().nuevoEmpleadorMatcheado(auxPEmpleador);
